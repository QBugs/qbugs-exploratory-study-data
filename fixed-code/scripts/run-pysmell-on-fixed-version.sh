...
# the fixed version of each bug listed in ../../subjects/data/generated/bugs-in-quantum-computing-platforms.csv
# writes PySmell's output to the provided output file.
#
# Usage:
# run-pysmell-on-fixed-version.sh
#   [--bugs_file_path <path, e.g., ../../subjects/data/generated/bugs-in-quantum-computing-platforms.csv>]
#   --output_dir_path <path, e.g., ../data/generated/pysmell-on-fixed-version>
#   [help]
# ------------------------------------------------------------------------------
...
USAGE="Usage: ${BASH_SOURCE[0]} [--bugs_file_path <path, e.g., ../../subjects/data/generated/bugs-in-quantum-computing-platforms.csv>] --output_dir_path <path, e.g., ../data/generated/pysmell-on-fixed-version> [help]"
...
while read -r row; do
  ...
  cd "$project_repository_dir_path"
    git checkout "$fix_commit_hash" || die "[ERROR] Commit '$fix_commit_hash' not found!"
  popd > /dev/null 2>&1
...
find "$OUTPUT_DIR_PATH" -type f -name "smells.csv" -exec tail -n +2 {} \;     >> "$OUTPUT_DIR_PATH/smells.csv"
