...
import org.codecomponents.commands.Commands;
...
/**
 * {@link org.kohsuke.args4j.spi.OptionHandler} which uses {@link org.command.CommandParser}
...
public class CommandHandler extends OptionHandler<Command> {
     ...
     * @param setter
     */
    public CommandHandler(final CmdLineParser parser, final OptionDef option, final Setter<Object> setter) {
        super(parser, new OptionDef(Commands.names(), "<command>", option.required(), option.help(), option.hidden(), CommandHandler.class, option.isMultiValued()) {
        }, setter);
    }
