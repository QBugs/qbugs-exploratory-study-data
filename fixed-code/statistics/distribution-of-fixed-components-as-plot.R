# ------------------------------------------------------------------------------
# This script plots the distribution of fixed components as boxplot, barplot, and
# UpSetR plot.
#
# Usage:
#   Rscript distribution-of-fixed-components-as-plot.R
#     <input data file, e.g., ../data/generated/fixed-code-data.csv>
#     <output pdf file, e.g., distribution-of-fixed-components-as-plot.pdf>
# ------------------------------------------------------------------------------
...
if (length(args) != 2) {
  stop('USAGE: Rscript distribution-of-fixed-components-as-plot.R <input data file, e.g., ../data/generated/fixed-code-data.csv> <output pdf file, e.g., distribution-of-fixed-components-as-plot.pdf>')
}
...
# Aggregate data
df$'count' <- 1
agg_count <- aggregate(x=count ~ project_full_name + bug_id + bug_type + fixed_component, data=df, FUN=sum)
...
boxplot_it <- function(df, label, facets=FALSE, fill=FALSE) {
  ...
  # Basic boxplot
  if (fill) {
    p <- ggplot(df, aes(x=fixed_component, y=count, fill=bug_type))
  } else {
    p <- ggplot(df, aes(x=fixed_component, y=count))
  }
...
p <- ggplot(df, aes(x=fixed_component, fill=bug_type)) + geom_bar(position=position_dodge(width=1))
# Change x axis label
p <- p + scale_x_discrete(name='')
# Change y axis label
p <- p + scale_y_continuous(name='# Occurrences (log10)', trans='log10')
...
p <- ggplot(aggregate(x=. ~ bug_id + bug_type + fixed_component, data=df, FUN=length), aes(x=fixed_component, fill=bug_type)) + geom_bar(position=position_dodge(width=1))
# Change x axis label
p <- p + scale_x_discrete(name='')
# Change y axis label
...
for (bug_type in unique(df$'bug_type')) {
  ...
  for (keep_order in c(TRUE, FALSE)) {
    ...
    a <- dcast(df[df$'bug_type' == bug_type, ], ... ~ fixed_component, value.var='fixed_component', fun.aggregate=length)
    ...
    a <- dcast(aggregate(x=. ~ bug_id + fixed_component, data=df[df$'bug_type' == bug_type, ], FUN=length),
               bug_id ~ fixed_component, value.var='fixed_component', fun.aggregate=length)
...
x <- kruskal.test(fixed_component ~ count, data=agg_count[agg_count$'bug_type' == 'Classical', ])
plot_label(paste(
...
x <- kruskal.test(fixed_component ~ count, data=agg_count[agg_count$'bug_type' == 'Quantum', ])
plot_label(paste(
